...
class FeatureExtractor:
    ...
    def features(self, text):
        ...
        d['com'] = com
        d['ex'] = ex
        # print spaces/len(tokens),
        # d['spaces'] = spaces/len(tokens)
        tot_len = 0
        st_words = 0
        for token in tokens:
            ...
            tot_len += len(token)
        # print token
        # d['last_len'] = len(token)
        d['avg_len'] = tot_len/len(token)
        # d['st_words'] = st_words
        d['tot_len'] = tot_len
        for token in tokens:
        ...
        d['syl'] = self.num_syllables(tokens[-1]) + self.num_syllables(tokens[0])
            # print syl
            # d['pron_count'] = pron_count
        d['adjectives'] = len(adjectives)
            # d['special_count'] = special_count
        d['stop'] = stop
        return d
